...
# if(ser.isnoavailable() == False):
    # ser.open()
isConnected = False    
def bukakoneksi(COM,BAUDRATE):
    global ser
    global isConnected
    global printGUI
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    colors_red = list(map(lambda x: (int(x[0] * 255), int(x[1] * 0), int(x[2] * 0)), colors))
    colors_green = list(map(lambda x: (int(x[0] * 0), int(x[1] * 255), int(x[2] * 0)), colors))
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if class_name == 'tidak lengkap':
                bbox_color = colors_red[0]
            if class_name == 'hand':    
                bbox_color = colors_red[8]
                #sendSerial()
            if class_name == 'mouth':    
                bbox_color = colors_red[9]
                #sendSerial()
                #warning = "Warning: Try to Sent Serial"
            if class_name == 'ear':    
                bbox_color = colors_red[10]
                #sendSerial()
            if class_name == 'eye':    
                bbox_color = colors_red[11]
                #sendSerial()
            if class_name == 'hair':    
                bbox_color = colors_red[12]
                sendSerial()
                warning = "Warning: Try to Sent Serial"
            if class_name == 'lengkap' :
                bbox_color = colors_green[1]
            if class_name == 'helm' :    
                bbox_color = colors_green[2]
            if class_name == 'earmuff' :    
                bbox_color = colors_green[3]
            if class_name == 'glasses' :    
                bbox_color = colors_green[4]
            if class_name == 'masker_industri' :    
                bbox_color = colors_green[5]
            if class_name == 'masker_biasa' :    
                bbox_color = colors_green[6]
            if class_name == 'gloves' :    
                bbox_color = colors_green[7]
